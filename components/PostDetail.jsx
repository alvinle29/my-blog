...
const PostDetail = ({ post }) => {
  const getContentFragment = (index, text, obj, type) => {
    ...
    switch (type) {
      ...
      case 'paragraph':
        return <p key={index} className="mb-8 md:text-base text-sm">{modifiedText.map((item, i) => <React.Fragment key={i}>{item}</React.Fragment>)}</p>;
      case 'heading-four':
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg lg:p-8 md:pb-12 pb-4 mb-8">
      ...
      <div className="px-4 lg:px-0">
        ...
        <h1 className="mb-8 md:text-3xl text-xl font-semibold">
          {post.title}
        </h1>
